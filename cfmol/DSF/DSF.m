...
CS = 'all'; % CS = 'all','Si','Siall','interf','SiO';
timestep = 0.00016;%0.0008 % in unit of ps, MD timestep
%Nstep = 21000;
qsampling = 1;
%filenm = 'Si_bulk'; %'SiO3nmV0.2_sp1';
filenm = 'Si_8x8x8_int_Ge_64_atoms_grid';
...
Nat = aa{1}
aa = textscan(fid,'%d%s%s',1,'headerlines',1);
Ntype = aa{1}
...
q = zeros(3,qsampling*Nxy+1); 
q(1,:) = (0:qsampling*Nxy)/(qsampling*Nxy); % set the k points (or q points)
qL = q; qT1 = q; qT2 = q;
qL = 2*pi*qL/a; 
%qT1 = qL; qT2 = qL;
%qT1(2,:) = 0; qt2(3,:) = 0;
qT1(2,:) = 2; qT1 = 2*pi*qT1/a; qT2(3,:) = 2; qT2 = 2*pi*qT2/a;
qall = [qL, qT1, qT2];
...
QdotR = zeros(Nstep,3*(qsampling*Nxy+1));
ID = 1:Nat;
for i = 1:Nstep
    if i == 1
...
fre = n/(dt*Nstep);
df = 1/(dt*Nstep)
Sfactor = zeros(Nstep/2,3*(qsampling*Nxy+1));

for i = 1:3*(qsampling*Nxy+1)
    Sfft = abs(fft(QdotR(:,i),Nstep));
    Sfactor(:,i) = Sfft(1:Nstep/2);
end
%%%%%%%%%%%%% end of time Fourier transform %%%%%%%%%%%

SL = Sfactor(:,1:qsampling*Nxy+1); SL = [fre',SL];
ST1 = Sfactor(:,qsampling*Nxy+2:2*(qsampling*Nxy+1)); ST1 = [fre',ST1];
ST2 = Sfactor(:,2*(qsampling*Nxy+1)+1:end); ST2 = [fre',ST2];

fid = fopen(['SFactor_',filenm,'_',CS,'.dat'],'w');

format = repmat('%15.6f ',[1,qsampling*Nxy+2]);
format = [format,'\n'];
fprintf(fid,'Longitudinal mode at q=(0:%d)/%d\n',qsampling*Nxy,qsampling*Nxy);
