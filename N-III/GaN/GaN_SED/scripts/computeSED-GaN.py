...
n1, n2, n3 = [4,8,100] #size of simulation cell
dk = 100 #k space mesh, number of points between speciak k points

steps = 400000 #run time
...
num, pos, masses, uc, types, a, c = mod.makeGaN(n1,n2,n3)
...
specialk = np.array([[0,0,0], #G
                     [0,0,0.5]]) #Z
klabel = np.array(('G','Z'))
kpoints, kdist = mod.makeKpoints(prim,specialk,dk) #get the input k space 
#array from a funtion
...
#    ids = ids-1
    cellvec = pos[ids[:,0],:] #coords of unit cell
    cellvec = cellvec[np.argsort(uc[ids][:,0]),:] #sort by unit cell
    #this is IN CASE the order of unit cell ids doesn't match. By design,
    #they SHOULD match.
...
sedg = mod.smoothSED(sed,win,(thz[1]-thz[0])*2e12*np.pi)
...
#plt.imshow(np.log(sedg),interpolation='hamming',cmap='jet',aspect='equal')
#plt.tight_layout()
#plt.show()
###
